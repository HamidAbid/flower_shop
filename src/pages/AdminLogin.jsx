...
const AdminLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    // For this example, we're using a simple hard-coded credential check
    try {
      // Simulate API call delay
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // Admin credentials check (in a real app, this would be validated on the server)
      if (email === 'admin@flowershop.com' && password === 'admin123') {
        // Save to Google Drive if remember me is checked and user is authorized
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-4 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100'}`}>
      <button
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className={`${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Please log in to continue</p>
        </div>
        ...
        <form onSubmit={handleLogin}>
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              ...
              />
              <label htmlFor="remember" className={`ml-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                Remember me
              </label>
            </div>
            ...
            <div>
              <button
                ...
                disabled={!googleApiInitialized || googleDriveAuthorized}
                className={`flex items-center text-sm ${
                  darkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-800'
                } ${(!googleApiInitialized || googleDriveAuthorized) && 'opacity-50 cursor-not-allowed'}`}
              >
                <FaGoogle className="mr-1" />
                {googleDriveAuthorized ? 'Google Drive Connected' : 'Connect Google Drive'}
              </button>
            </div>
          </div>
